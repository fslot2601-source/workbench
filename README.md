...
从 1.3.0 开始，Workbench 可以在应用菜单或“设置 → 关于”中检查更新，并默认定期检查 GitHub 上的新版本。更新 ZIP 会经过 Workbench 的 EdDSA 签名验证；没有通过验证的安装包不会被接受。

1.2.3 及更早版本尚未内置更新器，需要最后手动下载一次 1.3.0。关闭自动检查后，Workbench 不会在后台查询新版本，仍可随时手动检查。
