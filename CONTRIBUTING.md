...
发布候选包使用 `./scripts/build-release.sh` 生成；脚本会调用 `verify-release.sh` 解包 ZIP、挂载 DMG 并复核内部应用。本地单独复核未签名包时必须显式设置 `ALLOW_UNSIGNED=1`。公开分发模式必须同时使用 Developer ID Application 与 Apple 公证。自动更新包还必须使用维护者钥匙串中的 Workbench EdDSA 私钥签名；贡献者无需也不应获得这把私钥。完整步骤见 `docs/RELEASE_CHECKLIST.md`。
