...
我们做 Workbench，不是因为 Codex 的能力不够，而是因为许多能力藏得太深：配置散落在文件、协议和运行状态里，普通用户很难知道它们是什么、是否生效，更难放心地调整。

Workbench 想做的是 Codex 与用户之间的“可见层”。它不接管 Codex，而是把看不见的能力说明白，把真实状态展示出来，再把必要的控制权交还给用户。

### 1. 先看清，再操作

“写进配置”“开关已打开”“实际可用”是三件不同的事。Workbench 尽量读取 Codex 的实时结果，并明确区分已配置、已启用、已连接、已暴露能力和已观察到运行。无法确认时就显示“待确认”或“未验证”，不拿推测冒充成功。

### 2. 解释比开关更重要

一个功能如果只剩开关，用户仍然不知道自己在控制什么。因此 Skills 要说明用途和调用方式，Hooks 要说明触发时机和执行顺序，MCP 要展示它实际暴露的工具与资源，Memory 要说明内容、来源、范围和是否正在生效。

### 3. 不再制造一层黑箱

Workbench 优先使用 Codex App Server、Codex 配置和本机真实文件，不另造一套“看起来正常”的状态，也不虚构费用、调用历史或成功结果。界面展示的是当前证据能证明的事实，同时保留来源和诊断线索。

### 4. 自由不等于暴露复杂度

用户应该能调整 Skill 的隐式、显式和隐藏状态，管理 Hook 与 MCP，清理可安全重建的缓存，也应该能看到并理解 Memory。但自由不意味着把 TOML、JSON-RPC 和目录结构原样扔给用户。Workbench 提供安全、可撤回的常用控制；更个性化的 Memory 修改，则交给用户用自然语言告诉 Codex，而不是让界面悄悄改写它。

### 5. 中文优先，不是表面汉化

目标用户包括没有编程经验的人。Workbench 不只是翻译按钮，还会把 Codex 的概念、状态差异、触发逻辑和风险改写成普通中文；英文 Skill、MCP 与 Memory 内容也尽量提供自动翻译，同时允许查看原文。

### 6. 本地优先，边界清楚

Workbench 不建立自己的账户系统，不读取 Codex 或 GitHub 登录令牌，也不上传遥测。凭据继续由 Codex、GitHub CLI 和 macOS 钥匙串管理；Memory 默认只读，备份先脱敏预览，存储清理限定在可解释的白名单范围内。

最终原则很简单：**让 Codex 继续负责执行，让 Workbench 负责把它解释清楚。**
